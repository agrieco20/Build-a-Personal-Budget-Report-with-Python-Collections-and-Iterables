...
def test_task2_module5():
    ...
    expected_timeit_str = expected_timeit_str.replace(" ", "")
    expected_timeit_str_2 = expected_timeit_str_2.replace(" ", "")
    expected_timeit_str = expected_timeit_str.replace("\n", "")
    expected_timeit_str_2 = expected_timeit_str_2.replace("\n", "")
    ...
    for x in calls:
        temp = x.replace(" ", "")
        temp = temp.replace("\n", "")
        if (expected_timeit_str in temp or 
            expected_timeit_str_2 in temp):
...
def test_task3_module5():
    ...
    for x in calls:
        temp = x.replace(" ", "")
        temp = temp.replace("\n", "")
        if expected_timeit_str in temp:
            found_timeit_call = True
...
def test_task6_module5():
    ...
    try:
        for x in load_ast_tree('ExpenseCategories.py').body:
